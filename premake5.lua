...
newaction {
    trigger = "release",
    description = "HyperVTray: Creates a release of HyperVTray",
    execute = function ()
        ...
        local build_code = function (target)
            if have_msbuild then
                ...
                x64_ok = exec(have_msbuild .. " /m /v:q /p:configuration=release /p:platform=x64 hypervtray.sln /t:" .. target)
        ...
        os.chdir(src .. "/x64")
        if have_7z then
            --exec(have_7z .. " a -r  " .. target_dir .. "HyperVTray-v" .. version .. "-symbols.zip  *.pdb")
            exec(have_7z .. " a -r  " .. target_dir .. "HyperVTray-v" .. version .. ".zip  *.exe")
        end
...
newoption {
    trigger     = "nosign",
    description = "HyperVTray: don't sign the release files"
}

--------------------------------------------------------------------------------
newaction {
    trigger = "manifest",
    description = "HyperVTray: generate app manifest",
    execute = function ()
        toolchain = _OPTIONS["vsver"] or "vs2022"
